...
int main(){
    ...
    double omega=100.0; //frequency omega
    double m=0.5; //mass m
    ...
    for(int i=1; i<=THERM; i++){
        ...
        for(int t=0;t<T;t++){
            ...
            if(tau!=(T-1))
            {
                dS=(pow((site[tau+1]-new_site[tau]),2.0)+0.25*omega*omega*pow((site[tau+1]+new_site[tau]),2.0))-(pow((site[tau+1]-old_site[tau]),2.0)+0.25*omega*omega*pow((site[tau+1]+old_site[tau]),2.0));
                dS=(m/2.0)*dS;
            }
            else if(tau==(T-1)){
                dS=(pow((site[0]-new_site[tau]),2.0)+0.25*omega*omega*pow((site[tau+1]+new_site[tau]),2.0))-(pow((site[0]-old_site[tau]),2.0)+0.25*omega*omega*pow((site[tau+1]+old_site[tau]),2.0));
                dS=(m/2.0)*dS;
            }
